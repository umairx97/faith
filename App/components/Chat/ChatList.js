...
import { Text, View, ListItem, FlatList, BackHandler, Image, TouchableOpacity } from 'react-native'
...
export default class ChatList extends Component {
      ...
      .then(snapshot => {
        snapshot.forEach(childSnapshot => {
          ...
          if (this.state.loginUserId != key) {
            arr.push({ pName: userName, pUrl: childData, ids: userProfileId, age: getAge, gender: genderName });
          }
  ...
  //             key = childSnapshot.key;
  //             var kl = key;
  //           var aray= this.getUserProfileDetail(key)
          ...
          renderItem={({ item }) => (
            <View style={{ margin: 5 }}>
              <View style={{ flexDirection: 'row' }}>
                <View>
                  <TouchableOpacity onPress={()=>{this.onClickUser(item.ids)}}>
                    <Image style={{ height: 80, width: 80, margin: 3, resizeMode: 'cover', borderRadius: 40 }} source={{ uri: item.pUrl }}>
                    </Image>
                  </TouchableOpacity>
                </View>
                <View style={{ flexDirection: 'column', marginTop: 15 }}>
                  <View><Text style={{ fontSize: 15, marginLeft: 10 }}>{item.pName}</Text></View>
                  <View><Text style={{ fontSize: 12, marginLeft: 10 }}>{item.gender}</Text></View>
                  <View><Text style={{ fontSize: 12, marginLeft: 10 }}>{item.age}</Text></View>
                </View>
